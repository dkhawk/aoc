...
data class Vector(val x: Int = 0, val y: Int = 0) : Comparable<Vector> {
  ...
  fun inBounds(min: Vector, max: Vector): Boolean = x in min.x..max.x && y in min.y..max.y
...
fun Vector.manhattanDistanceTo(other: Vector): Int = abs(x - other.x) + abs(y - other.y)
...
fun Vector.toLongVector() = VectorLong(x.toLong(), y.toLong())
